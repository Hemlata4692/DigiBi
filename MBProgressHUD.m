...
- (void)layoutSubviews {
    ...
    CGRect textRect = [detailsLabel.text boundingRectWithSize:size
...
- (void)drawRect:(CGRect)rect {
	...
	CGRect myRect = CGRectMake(roundf((allRect.size.width - size.width) / 2) + self.xOffset,
								roundf((allRect.size.height - size.height) / 2) + self.yOffset, size.width, size.height);
	float radius = 10.0f;
	CGContextBeginPath(context);
    CGContextSetRGBFillColor(context, 3/255, 173.0/255, 194.0/255, 1.0);
    CGContextMoveToPoint(context, CGRectGetMinX(myRect) + radius, CGRectGetMinY(myRect));
    CGContextAddArc(context, CGRectGetMaxX(myRect) - radius, CGRectGetMinY(myRect) + radius, radius, 3 * M_PI / 2, 0, 0);
    CGContextAddArc(context, CGRectGetMaxX(myRect) - radius, CGRectGetMaxY(myRect) - radius, radius, 0, M_PI / 2, 0);
    CGContextAddArc(context, CGRectGetMinX(myRect) + radius, CGRectGetMaxY(myRect) - radius, radius, M_PI / 2, M_PI, 0);
    CGContextAddArc(context, CGRectGetMinX(myRect) + radius, CGRectGetMinY(myRect) + radius, radius, M_PI, 3 * M_PI / 2, 0);
