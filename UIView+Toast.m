...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        ...
        if([point caseInsensitiveCompare:@"top"] == NSOrderedSame) {
            ...
            return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - (toast.frame.size.height / 2)) - CSToastVerticalPadding-40);
...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    if (message != nil) {
        ...
        messageLabel.frame = CGRectMake(0.0, 50.0, expectedSizeMessage.width, expectedSizeMessage.height);
    }
    ...
    wrapperView.frame = CGRectMake(0.0, 50.0, wrapperWidth, wrapperHeight);
